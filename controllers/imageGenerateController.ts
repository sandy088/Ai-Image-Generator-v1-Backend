...
import { generateImageWithForeverUtils, upscaleImg } from "../utils/ImageGenerationUtils/imageGenerate";
dotenv.config();
const generateImage = async (req: Request, res: Response) => {
  try {
    ...
    const output: { error?: string }| undefined = await upscaleImg(prompt);
...
const generateImageWithForever = async (req: Request, res: Response) => {
  try {
    const { prompt } = req.body;
    ...
    const output: { error?: string }|undefined = await generateImageWithForeverUtils(prompt);
...
export { generateImage, generateImageWithForever };
